...
public class ImageProcessing {
	...
	public static int[][] stretchHorizontally(int[][] imageTwoD) {
		// TODO: Fill in the code for this method
    int[][] new2DArray = new int[imageTwoD.length][imageTwoD[0].length*2];
    int it = 0;
    for(int i=0; i<imageTwoD.length; i++){
      for(int j=0; j<imageTwoD[i].length; j++){
        it = j*2;
        new2DArray[i][it] = imageTwoD[i][j];
        new2DArray[i][it+1] = imageTwoD[i][j];
      }
    }
		return new2DArray;
	}
	public static int[][] shrinkVertically(int[][] imageTwoD) {
		// TODO: Fill in the code for this method
    int[][] new2DArray = new int[imageTwoD.length / 2][imageTwoD[0].length];
    for(int i=0; i<imageTwoD[0].length; i++){
      for(int j=0; j<imageTwoD.length; j+=2){
        new2DArray[j/2][i] = imageTwoD[j][i];
      }
    }
		return new2DArray;
	}
    ...
    for(int i=0; i<imageTwoD.length; i++){
      for(int j=0; j<imageTwoD.length; j++){
        new2DArray[i][j] = imageTwoD[(imageTwoD.length-1)-i][(imageTwoD[i].length-1)-j];
      }
    }
		return new2DArray;
	}
    ...
    for(int i=0; i<canvas.length; i++){
      for(int j=0; j<canvas[i].length; j++){
        ...
        int[] rgbaValue = {randRed, randGreen, randBlue, 255};
        canvas[i][j] = getColorIntValFromRGBA(rgbaValue);
      }
    }
		return canvas;
	}
